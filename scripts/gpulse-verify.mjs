...
checkTrue('conversation deletion still cascades to messages', /FOREIGN KEY \(conversation_id\)[\s\S]*ON DELETE CASCADE/.test(legal02bMigrationSource));
...
checkTrue('local deletion backend uses server-only environment secret', /Deno\.env\.get\('SUPABASE_ADMIN_KEY'\)/.test(deletionBackendSource) && !/service_role/i.test(deletionBackendSource));
