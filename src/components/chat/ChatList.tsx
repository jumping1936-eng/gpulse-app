  ...
  const fetchConversations = useCallback(async () => {
    try {
      ...
      const otherUserIds = Array.from(
        new Set(
          data
            .map((convo: Record<string, unknown>) => {
              const isUser1 = convo.user1_id === user.id;
              return isUser1 ? convo.user2_id : convo.user1_id;
            })
      ...
      if (otherUserIds.length > 0) {
        ...
        blockedByOthers = new Set((blockRows ?? []).map((row: { blocker_id: string }) => row.blocker_id));
      }
      ...
      const formattedConvos = data
        .map((convo: Record<string, unknown>): Conversation | null => {
          ...
          if (blockedByOthers.has(otherUserId)) return null;
          ...
          return {
            ...
            other_user: (otherUser as unknown as DBProfile) ?? { id: '', full_name: '探索新朋友', avatar_url: '', bio: '' },
