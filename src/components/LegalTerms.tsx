          ...
          {onAccept && (
            <div className="shrink-0 border-t border-white/10 px-4 py-4 sm:px-8">
              <div className="mx-auto max-w-3xl flex items-center justify-between">
                <p className="text-xs text-slate-500">{locale === 'zh-TW' ? '點擊接受即表示您同意上述條款' : 'Accepting indicates agreement to the above terms'}</p>
                <button
                  type="button"
                  onClick={onAccept}
                  className="rounded-xl bg-gradient-to-r from-violet-600 to-blue-600 px-6 py-2.5 text-sm font-semibold text-white transition hover:from-violet-500 hover:to-blue-500 shadow-lg shadow-violet-500/20"
                >
                  {locale === 'zh-TW' ? '我接受' : 'I Accept'}
                </button>
              </div>
            </div>
          )}
