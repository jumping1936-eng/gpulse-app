...
const ToggleSwitch = ({ isOn, onToggle }: { isOn: boolean, onToggle: () => void }) => (
  <button 
    onClick={onToggle} 
    className={`w-12 h-6 rounded-full transition-all relative shrink-0 ${isOn ? 'bg-violet-600' : 'bg-slate-700'}`}
  >
...
export default function ProfileView({ onOpenBlockedUsers }: ProfileViewProps) {
  const {
    ...
    setShowPaywall, setSimulateBlocked,
  } = useApp();
  ...
  const handleAccessAction = (userId: string, action: 'granted' | 'rejected' | 'revoked') => {
  ...
  function handleSaveProfile() {
    ...
    setProfile(editForm);
    if (editForm.publicPhotos.length > 0) setMyAvatar(editForm.publicPhotos[0]);
  ...
  const submitContactForm = () => {
    ...
    alert("✅ 訊息與附件已成功發送！客服團隊將於 1-3 個工作天內為您處理。");
    ...
    <div className="h-full overflow-y-auto bg-slate-950 pb-8 relative">
      ...
      <div className="pt-6 pb-4 flex flex-col items-center gap-3">
        <div className="w-full relative">
          <div className="flex overflow-x-auto snap-x snap-mandatory gap-4 px-6 no-scrollbar pb-4">
            {profile.publicPhotos.length > 0 ? (
              profile.publicPhotos.map((img, idx) => (
                <div key={idx} className="w-48 h-64 shrink-0 snap-center rounded-3xl overflow-hidden border-2 border-white/10 shadow-2xl relative bg-slate-800">
                  <img src={img} alt={`Public ${idx}`} className="w-full h-full object-cover" />
                  <div className="absolute top-2 right-2 bg-black/40 backdrop-blur-sm text-white/90 text-[10px] font-bold px-2 py-1 rounded-full">
                    {idx + 1} / {profile.publicPhotos.length}
                  </div>
                </div>
              ))
            ) : (
              <div className="w-48 h-64 shrink-0 snap-center rounded-3xl border-2 border-dashed border-white/20 bg-slate-800/50 flex flex-col items-center justify-center gap-2">
        ...
        <div className="text-center w-full px-4">
          ...
          <div className="flex items-center justify-center gap-3 mt-2">
            ...
            <button onClick={() => { if(!isVIP) { setShowPaywall(true); return; } setStealthMode(!stealthMode); }} className={`relative inline-flex h-6 w-[42px] items-center rounded-full transition-colors focus:outline-none ${stealthMode ? 'bg-slate-600' : 'bg-white/10'}`}>
      ...
      {!isVIP && (
        <div className="px-4">
          <button onClick={() => setShowPaywall(true)} className="w-full flex items-center justify-center gap-2 bg-gradient-to-r from-amber-500/20 to-orange-500/10 border border-amber-500/30 rounded-xl px-4 py-2.5 hover:border-amber-500/50 transition-all mt-2">
            ...
            <span className="text-amber-400 text-sm font-medium">升級為 VIP</span>
      ...
      {isEditModalOpen && (
        <div className="fixed inset-0 z-[100] bg-slate-950 flex flex-col animate-in slide-in-from-bottom-full duration-300">
          <div className="flex items-center justify-between px-4 py-4 bg-slate-900 border-b border-white/10 shrink-0">
            ...
            <button onClick={handleSaveProfile} className="text-violet-400 font-bold hover:text-violet-300 px-2 py-1 flex items-center gap-1">
              <Check className="w-4 h-4" /> 儲存
            </button>
          </div>

          <div className="flex-1 overflow-y-auto p-4 space-y-6 pb-24">
            <div className="bg-slate-900 border border-white/5 rounded-2xl p-4 space-y-5 relative overflow-hidden">
              ...
              <div>
                ...
                <div className="flex gap-3 overflow-x-auto pb-2 no-scrollbar relative z-10">
                  {editForm.publicPhotos.map((photo, idx) => (
                    <div key={idx} className="shrink-0 w-24 h-32 rounded-xl border border-white/10 relative group overflow-hidden bg-slate-800">
                  ...
                  {editForm.publicPhotos.length < 3 && (
                    <div onClick={() => triggerPhotoUpload('public')} className="shrink-0 w-24 h-32 rounded-xl bg-slate-800/50 border-2 border-dashed border-sky-500/30 flex flex-col items-center justify-center cursor-pointer hover:bg-slate-800 transition-colors"><Plus className="w-6 h-6 text-sky-500/70 mb-1" /></div>
                  )}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex gap-3 overflow-x-auto pb-2 no-scrollbar relative z-10">
                  {editForm.privatePhotos.map((photo, idx) => (
                    <div key={idx} className="shrink-0 w-24 h-32 rounded-xl border border-amber-500/30 relative group overflow-hidden bg-slate-800">
                  ...
                  <div onClick={(e) => { e.stopPropagation(); triggerPhotoUpload('private'); }} className="shrink-0 w-24 h-32 rounded-xl bg-amber-950/20 border-2 border-dashed border-amber-500/30 flex flex-col items-center justify-center cursor-pointer hover:bg-amber-900/30 transition-colors relative overflow-hidden">
                    {!isVIP && editForm.privatePhotos.length >= 2 ? <Crown className="w-6 h-6 text-amber-500/70 mb-1" /> : <Plus className="w-6 h-6 text-amber-500/70 mb-1" />}
            ...
            <div className="bg-slate-900 border border-white/5 rounded-2xl p-4 space-y-4">
              <div className="flex gap-4">
                <div className="flex-2">
                  ...
                  <input type="text" value={editForm.name} onChange={(e) => setEditForm({ ...editForm, name: e.target.value })} className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-white focus:border-violet-500 outline-none" />
                </div>
                <div className="flex-1">
                  ...
                  <input type="number" value={editForm.age} onChange={(e) => setEditForm({ ...editForm, age: e.target.value })} className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-white focus:border-violet-500 outline-none" />
                </div>
              </div>
              <div className="flex gap-4">
                <div className="flex-1">
                  ...
                  <input type="number" value={editForm.height} onChange={(e) => setEditForm({ ...editForm, height: e.target.value })} className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-white focus:border-violet-500 outline-none" placeholder="175" />
                </div>
                <div className="flex-1">
                  ...
                  <input type="number" value={editForm.weight} onChange={(e) => setEditForm({ ...editForm, weight: e.target.value })} className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-white focus:border-violet-500 outline-none" placeholder="65" />
                </div>
              </div>
              <div>
                ...
                <textarea value={editForm.bio} onChange={(e) => setEditForm({ ...editForm, bio: e.target.value })} rows={4} placeholder="介紹一下你自己吧..." className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-white focus:border-violet-500 outline-none resize-none" />
              </div>
            </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {ROLE_OPTIONS.map((option) => (
                    <button key={option} onClick={() => toggleArraySelection('role', option)} className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-all ${editForm.role.includes(option) ? 'bg-sky-600 text-white shadow-lg shadow-sky-500/30' : 'bg-slate-800 text-slate-400 border border-slate-700'}`}>{option}</button>
                  ))}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {LOOKING_FOR_OPTIONS.map((option) => (
                    <button key={option} onClick={() => toggleArraySelection('lookingFor', option)} className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${editForm.lookingFor.includes(option) ? 'bg-violet-600 text-white shadow-lg shadow-violet-500/30' : 'bg-slate-800 text-slate-400 border border-slate-700'}`}>{option}</button>
                  ))}
                </div>
              </div>
            </div>
            ...
            <div className="bg-slate-900 border border-white/5 rounded-2xl p-4 space-y-4">
              ...
              <div className="relative">
                ...
                <input type="text" value={editForm.instagram} onChange={(e) => setEditForm({...editForm, instagram: e.target.value})} placeholder="Instagram 帳號或網址" className="w-full bg-slate-950 border border-slate-800 rounded-xl pl-10 pr-4 py-3 text-white focus:border-pink-500 outline-none" />
              </div>
              <div className="relative">
                <Facebook className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-blue-500" />
                <input type="text" value={editForm.facebook} onChange={(e) => setEditForm({...editForm, facebook: e.target.value})} placeholder="Facebook 帳號或網址" className="w-full bg-slate-950 border border-slate-800 rounded-xl pl-10 pr-4 py-3 text-white focus:border-blue-500 outline-none" />
              </div>
              <div className="relative">
                <Twitter className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-sky-400" />
                <input type="text" value={editForm.twitter} onChange={(e) => setEditForm({...editForm, twitter: e.target.value})} placeholder="X (Twitter) 帳號或網址" className="w-full bg-slate-950 border border-slate-800 rounded-xl pl-10 pr-4 py-3 text-white focus:border-sky-400 outline-none" />
              </div>
              <div className="relative">
                <Send className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-blue-400" />
                <input type="text" value={editForm.telegram} onChange={(e) => setEditForm({...editForm, telegram: e.target.value})} placeholder="Telegram ID 或網址" className="w-full bg-slate-950 border border-slate-800 rounded-xl pl-10 pr-4 py-3 text-white focus:border-blue-400 outline-none" />
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-6">
            <div className="bg-slate-900 border border-white/5 rounded-2xl divide-y divide-white/5">
              <div className="px-4 py-5 flex items-center justify-between">
                ...
                <ToggleSwitch isOn={notifications.newMatch} onToggle={() => setNotifications({...notifications, newMatch: !notifications.newMatch})} />
              </div>
              <div className="px-4 py-5 flex items-center justify-between">
                <div>
                ...
                <ToggleSwitch isOn={notifications.newMessage} onToggle={() => setNotifications({...notifications, newMessage: !notifications.newMessage})} />
              </div>
              <div className="px-4 py-5 flex items-center justify-between opacity-80">
                ...
                {isVIP ? (
                  <ToggleSwitch isOn={notifications.profileLike} onToggle={() => setNotifications({...notifications, profileLike: !notifications.profileLike})} />
                ) : (
            ...
            <div className="bg-slate-900 border border-white/5 rounded-2xl divide-y divide-white/5">
              <div className="px-4 py-5 flex items-center justify-between">
                ...
                <ToggleSwitch isOn={notifications.appUpdates} onToggle={() => setNotifications({...notifications, appUpdates: !notifications.appUpdates})} />
              </div>
              <div className="px-4 py-5 flex items-center justify-between">
                <div>
                ...
                <ToggleSwitch isOn={notifications.emailPromo} onToggle={() => setNotifications({...notifications, emailPromo: !notifications.emailPromo})} />
              </div>
            </div>
          </div>
        </div>
      ...
      {isPrivacyModalOpen && (
        <div className="fixed inset-0 z-[100] bg-slate-950 flex flex-col animate-in slide-in-from-right duration-300">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-6 pb-20">
            ...
            <div className="bg-slate-900 border border-white/5 rounded-2xl px-4 py-5 flex items-center justify-between">
              <div>
              ...
              <ToggleSwitch isOn={profile.hideDistance} onToggle={() => setProfile({...profile, hideDistance: !profile.hideDistance})} />
            </div>
            ...
            <div className="bg-slate-900 border border-white/5 rounded-2xl p-4">
              ...
              <button 
                ...
                className="w-full bg-slate-800 hover:bg-slate-700 text-white text-sm font-medium py-2.5 rounded-xl flex items-center justify-center gap-2 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isDownloading ? (
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-6">
            ...
            <div className="bg-gradient-to-br from-violet-600/20 to-blue-600/20 border border-violet-500/30 rounded-2xl p-5 text-center flex flex-col items-center">
              ...
              <button 
                ...
                className="w-full bg-violet-600 hover:bg-violet-500 text-white font-semibold py-2.5 rounded-xl flex items-center justify-center gap-2 transition-colors shadow-lg shadow-violet-500/20"
              >
        ...
        <div className="fixed inset-0 z-[150] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-slate-900 border border-slate-700 w-full max-w-sm rounded-2xl p-5 shadow-2xl animate-in zoom-in-95 duration-200">
            ...
            <textarea 
              ...
              placeholder="請輸入您的問題..."
              rows={4}
              className="w-full bg-slate-950 border border-slate-700 rounded-xl px-4 py-3 text-white placeholder:text-slate-600 focus:outline-none focus:border-violet-500 focus:ring-1 focus:ring-violet-500 transition-all resize-none mb-4"
            />
            ...
            <div className="flex items-center gap-3">
              <button onClick={() => contactFileRef.current?.click()} className="p-2.5 rounded-xl bg-slate-800 text-slate-400 hover:text-white hover:bg-slate-700 transition-colors" title="附上截圖"><ImagePlus className="w-5 h-5" /></button>
              <button onClick={() => { setContactFormOpen(false); setContactMessage(''); setContactAttachment(null); }} className="flex-1 py-2.5 rounded-xl font-semibold text-slate-300 bg-slate-800 hover:bg-slate-700 transition-colors">取消</button>
              <button onClick={submitContactForm} className="flex-1 py-2.5 rounded-xl font-semibold text-white bg-violet-600 hover:bg-violet-500 transition-all shadow-lg shadow-violet-500/20 flex items-center justify-center gap-2"><Send className="w-4 h-4" /> 送出</button>
            </div>
          </div>
        </div>
      )}
      ...
      {reportFormState.isOpen && (
        <div className="fixed inset-0 z-[150] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-slate-900 border border-red-900/50 w-full max-w-sm rounded-2xl p-5 shadow-2xl animate-in zoom-in-95 duration-200">
            ...
            <textarea 
              ...
              rows={4}
              className="w-full bg-slate-950 border border-slate-700 rounded-xl px-4 py-3 text-white placeholder:text-slate-600 focus:outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500 transition-all resize-none mb-4"
            />
            ...
            <div className="flex items-center gap-3">
              <button onClick={() => reportFileRef.current?.click()} className="p-2.5 rounded-xl bg-slate-800 text-slate-400 hover:text-white hover:bg-slate-700 transition-colors" title="附上證據截圖"><ImagePlus className="w-5 h-5" /></button>
              <button onClick={() => { setReportFormState({ isOpen: false, reason: '', details: '' }); setReportAttachment(null); }} className="flex-1 py-2.5 rounded-xl font-semibold text-slate-300 bg-slate-800 hover:bg-slate-700 transition-colors">取消</button>
      ...
      <DevPanel onSimulateBlockedCountry={() => setSimulateBlocked(true)} />
    </div>
  );
}